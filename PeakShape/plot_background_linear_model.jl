...
det = chinfo.detector[65]
fit_func = :f_fit_bckSlope
...
function background_model(x, result_fit; background_slope = 0.0, background_center = 0.0 )
    v = strip_units(result_fit)
    if background_slope == 0 
        return background_peakshape(x, v.μ, v.σ, v.step_amplitude, v.background)
    else
        return background_peakshape(x, v.μ, v.σ, v.step_amplitude, v.background;  background_slope =  background_slope, background_center = background_center)
    end
end
...
for (i, peak) in enumerate(th228_names)
    ...
    background_slopefit(E) = background_model.(E, Ref(result_fit[peak]); background_slope = mvalue(result_fit[peak].background_slope), background_center = ps.peak_pos)
    background_slope(E, slope) = background_model.(E, Ref(result_fit[peak]); background_slope = slope, background_center = ps.peak_pos)
    
    # get pseudo prior for background slope
    local xslopes = range(-2, 0.5, length = 1000)
    local prior_cdf = cdf.(LegendSpecFits.get_pseudo_prior(h, ps, fit_func).background_slope, xslopes)
    local slope_min = xslopes[findmin(abs.(prior_cdf .- 0.01))[2]] 

    local ptmp = plot(xl, background_def(xl), fillrange = background_slope(xl, slope_min), 
            alpha = 0.5, fillcolor = :dodgerblue, color = :white, label = "Background-slope pseudoprior (99%)", linewidth = 0.1)
    plot!(xr,background_def(xr), fillrange = background_slope(xr, slope_min),
            alpha = 0.5, fillcolor = :dodgerblue, color = :white, label = false, linewidth = 0.1)        
    plot!(x, background_def(x) , label = "Background without slope", color = :black, lienwidth = 2)
    plot!(x, background_slopefit(x), label = "Background fit result", color = :darkorange, linewidth = 3, linestyle = :dash)
...
for (i, peak) in enumerate(th228_names)
    ...
    # get pseudo prior for background slope
    local xslopes = range(-2, 0.5, length = 1000)
    # prior_cdf = cdf.(LegendSpecFits.get_pseudo_prior(h, ps, fit_func).background_slope, xslopes)
    local prior_pdf = pdf.(LegendSpecFits.get_pseudo_prior(h, ps, fit_func).background_slope, xslopes)

    if i ==1 
        pplt = plot(xslopes, prior_pdf./trapz(xslopes,prior_pdf), linewidth = 2, label = "$peak")
    else
        plot!(xslopes, prior_pdf./trapz(xslopes,prior_pdf), linewidth = 2, label = "$peak")
    end
end
title!("Pseudo-prior distribution for background slope")
xlims!(-0.2,0.05)
xlabel!("Background slope (counts / keV)")
