# This script does:
# 1. Generate randomized MC spectra for different signal strengths (nSamples = samples per signal strength)
# 2. Fit each samples. Use 0nbb peakshape and maximum likelihood fit
# 3. Also fit with fixed signal strength to test Wilks theorem 
...
nSamples = 100000 
reCalc = true
signal_strength  = collect(0.0:0.1:5.0) # in units of 1e-26 yr^-1
v_input = (exposure = 200, efficiency = 0.95, fit_window = 240, Qbb = 2039.0, Qbb_bias = 0.0, Qbb_sigma = 2.5, sig_exp = 1.0e-26) 

sig = signal_strength[end]
