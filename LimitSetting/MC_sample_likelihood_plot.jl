# load results from script `MC_sample_likelihood.jl` and plot 
using Plots
using Statistics
using Measurements
...
sig = signal_strength[i]
    ...
    stephist(signal_fit, fill = true, color = :silver,
            ...
            ylabel = "Occurrence",
            label = @sprintf("Fit results samples; median = %.2f ± %.3f ", quantile(Measurements.value.(signal_fit), 0.5), std(signal_fit)/sqrt(length(signal_fit))),
            legend = :outertop)
    ylims!(0, ylims()[2])
